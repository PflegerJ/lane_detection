...
void cu_test_hysteresis(pixel_channel_t *in, pixel_channel_t *out, unsigned rows, unsigned cols)
{
	...
	unsigned pixel_count = rows * cols;
	size_t img_size = pixel_count * sizeof(pixel_channel_t);

	/* allocate device memory */
	cudaMalloc((void**)&in_pixels, img_size);
	cudaMalloc((void**)&out_pixels, img_size);
	cudaMalloc((void**)&idx_map, pixel_count * sizeof(idx_map[0]));

	/* copy original pixels to GPU device as in_pixels*/
	cudaMemcpy(in_pixels, in, img_size, cudaMemcpyHostToDevice);
	...
	/* launch kernels */
	cu_hysteresis_high << <pixel_count / 1024, 1024, 0, stream >> > (out_pixels, in_pixels, idx_map, t_high, rows, cols);
	cu_hysteresis_low << <pixel_count / 1024, 1024, 0, stream >> > (out_pixels, in_pixels, idx_map, t_low, rows, cols);

	/* copy blurred pixels from GPU device back to host as out_pixels*/
	cudaMemcpy(out, out_pixels, img_size, cudaMemcpyDeviceToHost);
