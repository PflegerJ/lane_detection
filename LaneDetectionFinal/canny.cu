...
__global__ void GaussianFilter(pixel_t* in_pixels, pixel_t* out_pixels, int rows, int cols, double* in_kernel, int kernel_dim)
{
    // calculate x and y index based off block and thread index
    int row = blockIdx.y * blockDim.y + threadIdx.y;
    int col = blockIdx.x * blockDim.x + threadIdx.x;

    int pixNum = row * col;
    // apply the Gaussian filter to the index
    if (pixNum >= 0 && pixNum < rows * cols)
    {
        double kernelSum;
        double redPixelVal;
        ...
        for (int i = 0; i < KERNEL_SIZE; ++i) {
            for (int j = 0; j < KERNEL_SIZE; ++j) {
                ...
                // check edge cases, if within bounds, apply filter
                if ((( + ((i - ((KERNEL_SIZE - 1) / 2)) * cols) + j - ((KERNEL_SIZE - 1) / 2)) >= 0)
                    && ((pixNum + ((i - ((KERNEL_SIZE - 1) / 2)) * cols) + j - ((KERNEL_SIZE - 1) / 2)) <= rows * cols - 1)
                    && (((pixNum % cols) + j - ((KERNEL_SIZE - 1) / 2)) >= 0)
                    && (((pixNum % cols) + j - ((KERNEL_SIZE - 1) / 2)) <= (cols - 1))) {
...
{
    CommandLineParser parser(argc, argv, "{@input | pika.jpg | input image}");
    //src = imread(samples::findFile(parser.get<String>("@input")), IMREAD_COLOR); // Load an image
    src = imread("C:\\HPCVideo\\pika.jpg");
    if (src.empty())
    {
